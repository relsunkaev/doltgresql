...
func Convert(postgresStmt parser.Statement) (vitess.Statement, error) {
	...
	case *tree.AlterConversion:
		return nodeAlterConversion(ctx, stmt)
	...
	case *tree.CreateAccessMethod:
		return nodeCreateAccessMethod(ctx, stmt)
	...
	case *tree.DropAccessMethod:
		return nodeDropAccessMethod(ctx, stmt)
	...
	case *tree.ReassignOwned:
		return nodeReassignOwned(ctx, stmt)
