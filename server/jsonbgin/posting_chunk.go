...
func EncodePostingChunk(rowRefs [][]byte) (PostingChunk, error) {
	...
	binary.BigEndian.PutUint32(payload[6:postingChunkV1HeaderSize], uint32(len(copied)))
	for _, rowRef := range copied {
	...
	chunk := PostingChunk{
		...
		RowCount:      uint32(len(copied)),
		RowRefs:       copied,
		Payload:       payload,
		Checksum:      checksum,
	}
	if len(copied) > 0 {
		chunk.FirstRowRef = append([]byte(nil), copied[0]...)
...
func DecodePostingChunk(payload []byte) (PostingChunk, error) {
	...
	case PostingChunkFormatVersionV1:
		return decodePostingChunkV1(payload)
	default:
		return PostingChunk{}, fmt.Errorf("unsupported JSONB GIN posting chunk payload version %d", version)
	}
}

func decodePostingChunkV1(payload []byte) (PostingChunk, error) {
	...
	for i := uint32(0); i < rowCount; i++ {
		...
		rowRef := append([]byte(nil), payload[offset:offset+int(length)]...)
		offset += int(length)
		if len(rowRefs) > 0 {
