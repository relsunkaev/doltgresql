...
func TestErrMessageToSQLStateFormatsDropTableForeignKeyDependency(t *testing.T) {
	...
	code, ok = errMessageToSQLState(`cannot drop index "fk_drop_index_parent_code_key" because constraint "fk_drop_index_parent_code_key" on table "fk_drop_index_parent" requires it`)
	require.True(t, ok)
	require.Equal(t, pgcode.DependentObjectsStillExist.String(), code)
}
...
func TestErrMessageToSQLStateFormatsMissingCommentObjects(t *testing.T) {
	for _, tt := range []struct {
		...
		{msg: `foreign-data wrapper "missing_grant_fdw" does not exist`, code: pgcode.UndefinedObject},
		...
		{msg: `server "missing_grant_server" does not exist`, code: pgcode.UndefinedObject},
