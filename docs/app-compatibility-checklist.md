...
- [x] Use deterministic unary numeric `floor(expr)` / `ceil(expr)` predicates
  ...
  signed integer inputs, and `ON CONFLICT` arbiter inference accepts the
  exact-expression shape; raw source-value predicates and wrong rounded values
...
- [x] Use deterministic `replace(expr, from, to)` predicates in partial-index
  ...
  DML enforcement evaluates the same empty-pattern and replacement behavior as
  the runtime function, and `ON CONFLICT` arbiter inference accepts the exact-
  expression shape; wrong replacement arguments, result values, and raw string
...
- [x] Use deterministic `split_part(expr, delimiter, field)` predicates in
  ...
  serialize as comparable predicate keys, planner filters can deparse them,
  partial unique DML enforcement evaluates the same positive, negative,
  out-of-range, empty-delimiter, and zero-field behavior as the runtime
  function, and `ON CONFLICT` arbiter inference accepts the exact-expression
  shape; wrong delimiters, field positions, result values, and raw source
  predicates remain rejected. Coverage in
  server/indexpredicate/implication_test.go, testing/go/functions_test.go,
  testing/go/partial_expression_index_test.go, and
  testing/go/insert_on_conflict_test.go. Tracked by dg-7ug.8.10.14.
- [x] Use deterministic `ascii(expr)` predicates in partial-index implication
  paths. Unary `ascii(...)` calls now serialize as comparable predicate keys,
  planner filters can deparse them, partial unique DML enforcement evaluates
...
- [x] Use deterministic same-expression `concat(...)` predicates in
  ...
  comparable predicate keys, planner filters can deparse them, partial unique
  DML enforcement evaluates PostgreSQL-style NULL-ignoring concatenation for
  supported scalar predicate values, and `ON CONFLICT` arbiter inference accepts
  the exact-expression shape; wrong arguments, result values, and raw
  source-column semantic rewrites remain rejected. Coverage in
  server/indexpredicate/implication_test.go,
  testing/go/partial_expression_index_test.go, and
...
- [~] Null ordering in indexes - `ASC NULLS LAST` / `DESC NULLS
  ...
  and NULLS LAST index scan ordering are metadata-preserved, but the btree
  planner boundary now exposes the supported physical/logical scan shapes:
  `NOT NULL` DESC keys, native nullable `ASC NULLS FIRST` /
  `DESC NULLS LAST`, and simple nullable `ASC NULLS LAST` /
  `DESC NULLS FIRST` indexes normalized through hidden generated null-probe
  keys. Unsupported sort-option shapes stay fenced from sort-elision, and
  ordinary predicate lookup through physically column-prefixed indexes remains
  available. Pinned by
...
- [x] Enable ordered-scan planning for nullable btree sort-option indexes whose
  ...
  planner-visible and return correct nullable ordering; non-native nullable
  null placement is covered by dg-7ug.8.3.4. Pinned by
  testing/go/index_benchmark_test.go and
...
- [x] Model the remaining simple-column nullable NULLS FIRST/LAST index scan
  ordering. Non-concurrent nullable `ASC NULLS LAST` and `DESC NULLS FIRST`
  btree indexes are normalized with hidden generated `col IS NULL` keys before
  the logical indexed column, and the planner exposes the logical
  `col IS NULL, col` prefix only when the hidden key is present. All four
...
- [x] Migration-tool introspection - `drizzle-kit introspect`
  ...
  verifies ORM CRUD plus commit / rollback afterward. TypeORM's real migration
  CLI runs `migration:run` against Doltgres using
  `QueryRunner` schema-builder migration files, records its migration metadata
  table, creates FK / JSONB / text[] / decimal schema plus indexes, and
  verifies post-migrate CRUD plus commit / rollback. Covered by
  testing/go/drizzle_kit_introspect_test.go,
  testing/go/prisma_db_pull_test.go,
  testing/go/alembic_autogenerate_test.go,
  testing/go/django_migration_test.go,
  testing/go/knex_migration_test.go,
  testing/go/active_record_migration_test.go, and
  testing/go/typeorm_migration_test.go. Tracked by dg-7ug.10.2.1,
  dg-7ug.10.2.2, dg-7ug.10.2.3, and dg-7ug.10.2.4.
