...
| Unique constraints and standalone unique indexes | semantic and catalog support | `testing/go/index_test.go`: generated/explicit names, `DROP INDEX` and `DROP CONSTRAINT` ownership boundaries; `testing/go/pgcatalog_test.go`: constraint/index catalog fixtures | Broader `ON CONFLICT` arbiter parity remains tracked by the uniqueness lane. |
