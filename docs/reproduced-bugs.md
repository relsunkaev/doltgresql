...
### Text domain typmod CHECK constraints see uncoerced values

- Reproducer: `TestTextDomainTypmodCheckUsesCoercedValueRepro` in
  `testing/go/domain_correctness_repro_test.go`.
- Command: `CGO_CPPFLAGS=-I/opt/homebrew/opt/icu4c@78/include
  CGO_LDFLAGS=-L/opt/homebrew/opt/icu4c@78/lib go test -vet=off ./testing/go
  -run TestTextDomainTypmodCheckUsesCoercedValueRepro -count=1`.
- Expected PostgreSQL behavior: text-domain `CHECK` constraints are evaluated
  after the domain base-type typmod is applied. A `varchar(3)` domain with
  `CHECK (length(VALUE) = 3)` accepts `abc   ` because the checked value is
  `abc`, and a `character(3)` domain with `CHECK (octet_length(VALUE) = 3)`
  accepts `ab` after padding.
- Observed Doltgres behavior: the insert is rejected by the `varchar(3)` domain
  check because `length(VALUE)` sees `6`, so text-domain validation runs before
  the declared base typmod is applied.
...
- Command: `CGO_CPPFLAGS=-I/opt/homebrew/opt/icu4c@78/include
  CGO_LDFLAGS=-L/opt/homebrew/opt/icu4c@78/lib go test -vet=off ./testing/go
  -run TestTextDomainTypmodBulkWritesUseCoercedValuesRepro -count=1`.
- Expected PostgreSQL behavior: `UPDATE ... FROM` and `COPY FROM` writes into
  `varchar(3)` and `character(3)` domain columns apply the domain base-type
  typmods before storage. `abc   ` stores as `abc`, and `ab` stores as a padded
  `character(3)` value.
- Observed Doltgres behavior: both write paths store `abc   ` with
  `length = 6` and underpadded `ab` with `octet_length = 2`, so joined and bulk
  writes can persist text-domain values outside their declared base typmods.
...
- Command: `CGO_CPPFLAGS=-I/opt/homebrew/opt/icu4c@78/include
  CGO_LDFLAGS=-L/opt/homebrew/opt/icu4c@78/lib go test -vet=off ./testing/go
  -run TestCompositeTypeDomainFieldEnforcesConstraintsRepro -count=1`.
- Expected PostgreSQL behavior: a composite type whose attribute is a domain
  accepts valid field values, persists the composite row, and rejects invalid
  field values with the domain constraint error.
- Observed Doltgres behavior: creating the domain, composite type, and table
  succeeds, but `INSERT INTO composite_field_items VALUES (ROW(1))` panics while
  resolving the assignment cast with a nil-pointer dereference in
  `server/functions/framework.getCast`. Invalid field values panic through the
  same path instead of reporting `composite_field_positive_check`.
