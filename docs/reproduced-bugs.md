...
- Reproducers: `TestGeneratedAlwaysIdentityOverridingSystemValueRepro` and
  `TestGeneratedByDefaultIdentityOverridingUserValueRepro` in
  `testing/go/sequence_dependency_repro_test.go`.
- Command: `CGO_CPPFLAGS=-I/opt/homebrew/opt/icu4c@78/include
  CGO_LDFLAGS=-L/opt/homebrew/opt/icu4c@78/lib go test ./testing/go -run
  'TestGenerated(AlwaysIdentityOverridingSystemValue|ByDefaultIdentityOverridingUserValue)Repro'
  -count=1`.
- Expected PostgreSQL behavior: `OVERRIDING SYSTEM VALUE` allows explicit
  values for `GENERATED ALWAYS AS IDENTITY` inserts, and `OVERRIDING USER
  VALUE` ignores caller-supplied identity values and uses the generated value.
- Observed Doltgres behavior: both inserts fail during parsing with `at or
  near "overriding": syntax error`.
