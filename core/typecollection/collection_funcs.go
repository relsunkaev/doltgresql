...
func (pgs *TypeCollection) UpdateRoot(ctx context.Context, root objinterface.RootValue) (objinterface.RootValue, error) {
	...
	if currentCount, currentCountErr := m.Count(); currentCountErr == nil && currentCount == 0 {
		hashOnRoot, err := pgs.LoadCollectionHash(ctx, root)
		if err != nil {
			return nil, err
		}
		if hashOnRoot.IsEmpty() {
			// Preserve roots that have never stored a type collection rather than
			// materializing an empty map for a no-op type finalization.
			return root, nil
		}
	}
