...
The schema-split harness starts a real `postgres:16-alpine` shard 0 plus a Doltgres shard 1 behind one PgDog logical database. It configures `[[sharded_schemas]]` for `shared.*`, configures `customer.orders` as a `customer_id` sharded table with a default mapping to shard 0, then verifies fully qualified `shared.*` traffic routes only to Postgres and unmigrated `customer.*` traffic stays on shard 0 while Doltgres remains clean. It then adds a list mapping for one customer ID, restarts PgDog, and verifies post-cutover writes for that customer route only to Doltgres.
