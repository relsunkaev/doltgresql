...
| Resharding and cutover | Doltgres exposes PostgreSQL replication source mode for `IDENTIFY_SYSTEM`, logical slot create/drop, `START_REPLICATION`, keepalives, standby status updates, and local sender/slot catalogs. It does not yet emit pgoutput row-change messages. | Replication connection and health probes can be smoke-tested. Do not rely on PgDog data resharding/cutover until row-change streaming exists. |
