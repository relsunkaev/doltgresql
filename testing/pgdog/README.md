...
| --- | --- | --- |
| 2PC / prepared transactions | `PREPARE TRANSACTION`, `COMMIT PREPARED`, `ROLLBACK PREPARED`, and `pg_prepared_xacts` are supported. Prepared transaction metadata and the prepared Dolt working set are recovered after a Doltgres restart. | PgDog 2PC can be smoke-tested across restart. Recovered `COMMIT PREPARED` is conservative: it refuses to apply if the target working set changed after `PREPARE TRANSACTION`, so resolve or roll back the prepared transaction before retrying cutover. |
