...
The schema-split harness starts a real `postgres:16-alpine` shard 0 plus a Doltgres shard 1 behind one PgDog logical database. It configures `[[sharded_schemas]]` for `shared.*`, configures `customer.orders` as a `customer_id` sharded table with a default mapping to shard 0, then verifies fully qualified `shared.*` traffic routes only to Postgres and unmigrated `customer.*` traffic stays on shard 0 while Doltgres remains clean. It copies the selected customer into Doltgres with table `COPY`, applies post-copy insert/update/delete changes, validates row and checksum convergence, then adds a list mapping for one customer ID, restarts PgDog, verifies post-cutover writes for that customer route only to Doltgres, and reverse-applies post-cutover insert/update/delete changes back to shard 0 across a Doltgres restart.
