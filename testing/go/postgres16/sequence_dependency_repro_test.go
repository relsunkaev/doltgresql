...
func TestAlterTableAddIdentityUsesTableOwnerRepro(t *testing.T) {
	RunScripts(t, []ScriptTest{
		{
			Name: "ALTER TABLE ADD IDENTITY sequence uses table owner",
			SetUpScript: []string{
				`CREATE ROLE identity_table_owner;`,
				`CREATE TABLE owned_identity_items (
					id INT NOT NULL,
					label TEXT
				);`,
				`ALTER TABLE owned_identity_items OWNER TO identity_table_owner;`,
			},
			Assertions: []ScriptTestAssertion{
				{
					Query: `ALTER TABLE owned_identity_items ALTER COLUMN id
						ADD GENERATED BY DEFAULT AS IDENTITY (
							SEQUENCE NAME owned_identity_items_id_seq
							START WITH 101
							INCREMENT BY 1
							NO MINVALUE
							NO MAXVALUE
				...
				},
				{
					Query: `SELECT pg_get_userbyid(relowner)
						...
						WHERE oid = 'owned_identity_items_id_seq'::regclass;`,
					Expected: []sql.Row{{"identity_table_owner"}},
				},
				{
					Query: `INSERT INTO owned_identity_items (label)
						VALUES ('after identity restore') RETURNING id;`,
...
// TestOnConflictDoUpdateConsumesNextvalDefaultRepro reproduces a sequence
// correctness bug: PostgreSQL evaluates INSERT defaults before conflict
// handling, so a nextval() default is consumed even when ON CONFLICT DO UPDATE
// updates an existing row instead of inserting the proposed row.
func TestOnConflictDoUpdateConsumesNextvalDefaultRepro(t *testing.T) {
	RunScripts(t, []ScriptTest{
		{
			Name: "ON CONFLICT DO UPDATE consumes nextval default",
			SetUpScript: []string{
				`CREATE SEQUENCE conflict_update_nextval_seq;`,
				`CREATE TABLE conflict_update_nextval_items (
					id INT PRIMARY KEY DEFAULT nextval('conflict_update_nextval_seq'),
					label TEXT UNIQUE,
					touched INT NOT NULL DEFAULT 0
				);`,
				`INSERT INTO conflict_update_nextval_items (label) VALUES ('kept');`,
			},
			Assertions: []ScriptTestAssertion{
				{
					Query: `INSERT INTO conflict_update_nextval_items (label)
						VALUES ('kept')
